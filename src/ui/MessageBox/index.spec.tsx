...
describe("<MessageBox/>", () => {
  ...
  test("renders title and message from props", () => {
    const { getByText } = render(<MessageBox {...props} />);
  ...
  test("applies a class name based on the variant prop", () => {
    const { container, rerender } = render(<MessageBox {...props} />);

    expect(container.firstElementChild?.classList.contains("default")).toBe(
      true,
    );
    ...
    expect(container.firstElementChild?.classList.contains("success")).toBe(
      true,
    );
  });
});
