  ...
  return (
    <div className={s.root}>
      <Menu fixed="top" inverted size="large" className={s.menu}>
        <Container>
          <Menu.Item onClick={() => props.history.push(PATH_FEED)}>Home</Menu.Item>
              ...
              {({ data }) => (
                <Dropdown item simple text={data && data.viewer ? data.viewer.name : 'Loading...'}>
                  <Dropdown.Menu>
                    ...
                    <Link to={PATH_CHOOSE_TEAM} className="item" style={{ color: 'black' }}>
                      <Icon name="exchange" />
                      Switch team
                    </Link>
                    <Dropdown.Item onClick={() => logout(props.history)}>
                      <Icon name="log out" />
                      Log out
                    </Dropdown.Item>
                  </Dropdown.Menu>
