...
describe('<CreatePost />', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = mount(withMockedProviders(<CreatePost back={false} />, mocks, mockCache, true));
  });
  ...
  it('shows a message if the amount is null', async () => {
    ...
    await act(async () => {
      component.setState({ amount: '', receivers: ['1'], message: 'Some message' });
      await wrapper.update();

      findByTestId(wrapper, 'submit-button').hostNodes().simulate('submit');
      await wrapper.update();

      expect(findByTestId(wrapper, 'error-message').text()).toBe('Amount can\'t be empty or 0.');
    });
  });
  ...
  it('shows a message if there are no receivers', async () => {
    ...
    await act(async () => {
      component.setState({ amount: '5', receivers: [], message: 'Some message' });
      await wrapper.update();

      findByTestId(wrapper, 'submit-button').hostNodes().simulate('submit');
      await wrapper.update();

      expect(findByTestId(wrapper, 'error-message').text()).toBe('You must select at least one receiver.');
    });
  });
  ...
  it('shows a warning if the message is too short', async () => {
    ...
    await act(async () => {
      ...
      expect(findByTestId(wrapper, 'error-message').text()).toBe('Message must be at least 4 characters.');
    });
  });
  ...
  it('shows a warning if the message is too long', async () => {
    ...
    await act(async () => {
      ...
      component.setState({ amount: '5', receivers: ['1'], message: 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.' });
      await wrapper.update();

      findByTestId(wrapper, 'submit-button').hostNodes().simulate('submit');
      await wrapper.update();

      expect(findByTestId(wrapper, 'error-message').text()).toBe('Message can have a maximum of 140 characters.');
    });
  });
  ...
  it('shows when the mutation is loading', async () => {
    ...
    await act(async () => {
      ...
      expect(findByTestId(wrapper, 'submit-button').hostNodes().hasClass('loading')).toBe(true);
    });
  });

  it('shows when there is an error', async () => {
    wrapper = mount(withMockedProviders(<CreatePost back={false} />, mocksWithError, mockCache, true));
    const component = wrapper.find('CreatePost').instance();
