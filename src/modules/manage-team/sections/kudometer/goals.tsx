...
import {
  Button, Divider, Form, Message, Popup, Table,
    ...
    return (
      <div>
        ...
        <Mutation<CreateGoalParameters>
          mutation={this.state.editingGoal ? UPDATE_GOAL : CREATE_GOAL}
          onCompleted={() => {
            toast.info(
              this.state.editingGoal ? 'Goal updated successfully!' : 'Goal created successfully!',
            );
            this.setState({
              goalName: '',
              goalKudos: '',
        ...
        >
          {(mutate, { error, loading }) => {
            ...
            return (
              <Form
                onSubmit={() => (this.state.editingGoal
                  ? mutate({
                    variables: {
              ...
              >
                <Form.Group widths="equal">
                  <Form.Input
                    ...
                    required
                    label="Name"
                    name={this.state.editingGoal ? 'editGoalName' : 'goalName'}
                    placeholder="Name"
                    value={this.state.editingGoal ? this.state.editGoalName : this.state.goalName}
                  ...
                  <Form.Input
                    ...
                    required
                    label="Kudos"
                    name={this.state.editingGoal ? 'editGoalKudos' : 'goalKudos'}
                    type="number"
                    min="100"
                    placeholder="Kudos"
                    value={this.state.editingGoal ? this.state.editGoalKudos : this.state.goalKudos}
                    onChange={this.handleChange}
                  />
                </Form.Group>
                <Button color="blue" loading={loading} disabled={loading} type="submit">
                  {this.state.editingGoal ? 'Update goal' : 'Create goal'}
                </Button>
                {this.state.editingGoal && (
                <Button color="orange" onClick={() => this.setState({ editingGoal: false })}>
                  Cancel
                </Button>
                )}
                {displayError && (
